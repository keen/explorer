...
export const updateUISavedQuery = ({ item, schemas }) => {
  ...
  const chartType = metadata ? translateDeprecatedCharts(metadata.visualization.chart_type) : getChartType(item);
  ...
  return {
    ...
    payload: {
      ...
      savedQuery: {
        ...
        displayName: metadata ? metadata.display_name : item.query_name,
        exists: true,
