    ...
    const filters = params.filters.map(
      ({ propertyName, propertyType, operator, propertyValue }) => {
        ...
        if (!propertyType) {
          ...
          if (typeof propertyValue === 'string' && new Date(propertyValue) && new Date(propertyValue).toString() !== 'Invalid Date') {
          ...
          if (propertyValue !== null && typeof propertyValue === 'object') { // array or object
        ...
        if (operator === 'Null') { // there is no 'null' operator but propertyType
          value = null;
        }
