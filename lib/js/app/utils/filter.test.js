...
describe('convertFilterValuesToJsonValues()', () => {
  test('params without filters return params', () => {
    const params = {
      actorProperty: 'geo_information.country',
  ...
  test('params without propertyType return nothing', () => {
    const params = {
      ...
      filters: [
        {
          propertyName: 'geo_information.country_code',
          operator: 'eq',
          propertyValue: 'pl',
        },
      ],
  ...
  test('params with "contains" operator return params', () => {
    const params = {
      ...
      filters: [
        {
          propertyName: 'geo_information.country',
          propertyType: 'String',
          operator: 'contains',
          propertyValue: 'PL',
        },
  ...
  test('params with "Datetime" as propertyType return params', () => {
    const params = {
      ...
      filters: [
        {
          propertyName: 'keen.created_at',
          propertyType: 'Datetime',
          operator: 'gt',
          propertyValue: '2020-06-01T12:00:00.000Z',
        },
      ],
      inverted: false,
      optional: false,
  ...
  test('params with "Boolean" as propertyType return boolean', () => {
    ...
    result.filters.forEach((filter) =>
      expect(typeof filter.propertyValue).toBe('boolean')
    );
  });
  ...
  test('params with "Number" as propertyType and "eq" as operator return number', () => {
    ...
    const result = convertFilterValuesToJsonValues(params);
    result.filters.forEach((filter) =>
      expect(typeof filter.propertyValue).toBe('number')
    );
  });
  ...
  test('params with "List" as propertyValue and "within" as operator return formatted coordinates', () => {
    const params = {
      ...
      filters: [
        {
          propertyName: 'geo_information.coordinates',
          operator: 'within',
          propertyType: 'List',
          propertyValue: { coordinates: ['10', '10'], maxDistanceMiles: '10' },
        },
      ],
      inverted: false,
      optional: false,
