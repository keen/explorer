...
class EmbedHTML extends Component {
  renderIfDefined({ key, value }) {
    const { savedQuery } = this.props.ui;
...
`;
  ...
  render() {
    ...
    let params = [];
    params.push(
      this.renderIfDefined({ key: 'analysisType', value: `'${analysisType}'` })
    );
    params.push(
      this.renderIfDefined({
        key: 'eventCollection',
    ...
    params.push(
      this.renderIfDefined({ key: 'interval', value: `'${interval}'` })
    );
    params.push(
      this.renderIfDefined({
        key: 'groupBy',
    ...
    params.push(
      this.renderIfDefined({ key: 'timezone', value: `${timezone}` })
    );
    params.push(
      this.renderIfDefined({
        key: 'percentile',
        value: `${JSON.stringify(percentile)}`,
      })
    );
    ...
    if (analysisType === 'extraction') {
      params.push(
        this.renderIfDefined({
          key: 'propertyNames',
          value: `${JSON.stringify(propertyNames)}`,
        })
      );
      params.push(
        this.renderIfDefined({
    ...
    if (analysisType === 'funnel') {
      ...
      steps.forEach((step) => {
        ...
        paramsForStep.push(`    {\n`);
        paramsForStep.push(
          this.renderIfDefinedInStep({
            key: 'actorProperty',
            value: `'${step.actorProperty}'`,
            step,
          })
        );
        paramsForStep.push(
        ...
        paramsForStep.push(
          this.renderIfDefinedInStep({
            key: 'timeframe',
            value: `${JSON.stringify(step.timeframe)}`,
            step,
          })
        );
        paramsForStep.push(
        ...
        paramsForStep.push(
          this.renderIfDefinedInStep({
            key: 'filters',
            value: `${JSON.stringify(step.filters)}`,
            step,
          })
        );
        paramsForStep.push(
    ...
    <body>
      ...
      <script>
        const chart = new KeenDataviz({
          ...
          type: '${chartType}',
          title: false ${stepLabels && stepLabelsString ? stepLabelsString : ''}
        });
...
${params.join('')}          })
    ...
    const prismedHtml = Prism.highlight(
      code,
...
export default connect(mapStateToProps, mapDispatchToProps)(EmbedHTML);
